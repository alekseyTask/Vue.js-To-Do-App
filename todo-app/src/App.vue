  ...
  <div id="app">
    <div class='content'>
      <!-- тут какогото хера (ну хтмл всетаки) непашет // 
           ...
           Откуда беруться эти названия тегов (напр TodoList и  create-todo)?
           -доподленно неизвестно, однако мы предполагаем следующие:
            ...
            Что за хрень (v-bind:todos="todos")?
            -v-bind Динамически связывает атрибуты тега или входной параметр компонента с выражением.
             в нашем случае <TodoList v-bind:todos="todos"></TodoList> мы взяли компонент  TodoList
             добрались до переменной todos и связали её с todos которая ретурниться из даты.
...
export default {
  ...
  // тут пишеться заполнение данных, мы просто заполняем массив делл а v-bind:todos="todos"
